...
insert into roles (id, code, name, permissions_jsonb)
values
  ('33333333-3333-3333-3333-333333333331', 'ADMIN', 'Administrador', '["members:write","payments:write","sync:write"]'::jsonb),
  ('33333333-3333-3333-3333-333333333332', 'STAFF', 'Staff', '["members:read","attendance:write"]'::jsonb),
  ('33333333-3333-3333-3333-333333333333', 'MEMBER', 'Miembro', '["profile:read"]'::jsonb)
on conflict (code) do nothing;
...
values (
  ...
  '{
    ...
    "apellido":"Lopez",
    "telefono":"55551111",
    "fechaInicio":"2026-03-12T00:00:00Z",
    "fechaFin":"2026-04-11T00:00:00Z",
    "meses":1,
    "diaPago":17,
    "asistencias":["2026-03-15T06:00:00Z","2026-03-16T06:00:00Z"],
    "pagos":[{"id":"77777777-7777-7777-7777-777777777771","fecha":"2026-03-12T12:00:00Z","monto":125.0,"meses":1}],
    "progreso":[{"imagePath":"https://images.unsplash.com/photo-1517836357463-d25dfeac3438?auto=format&fit=crop&w=900&q=80","fecha":"2026-03-12T12:00:00Z"}]
  }'::jsonb
)
...
insert into routines (id, tenant_id, branch_id, code, name, definition_jsonb)
values (
  ...
  'FULLBODY-START',
  'Full Body Inicial',
  '{"daysPerWeek":3,"blocks":[{"name":"Calentamiento","minutes":10}]}'::jsonb
)
on conflict (tenant_id, code) do nothing;

insert into configuration_entries (tenant_id, branch_id, config_key, value_jsonb)
values
  ('11111111-1111-1111-1111-111111111111', null, 'client.sync', '{"intervalMinutes":60,"promptOnOnline":true}'::jsonb),
  ('11111111-1111-1111-1111-111111111111', '22222222-2222-2222-2222-222222222222', 'pricing.default', '{"monthly":125.00}'::jsonb)
on conflict (tenant_id, branch_id, config_key) do nothing;

insert into app_versions (id, channel, version_name, minimum_supported_version, rollout_jsonb)
values (
  'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1',
  'web',
  '1.0.0',
  '1.0.0',
  '{"updateAvailable":false}'::jsonb
)
on conflict (channel, version_name) do nothing;
